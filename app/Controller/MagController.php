 ...
 */
class MagController extends AppController {
 ...
 * @var array
 */
	public $uses = array('Post', 'Tag', 'Media','PostComment');
 ...
 */
	...
	public function article($id) {
		$this->Post->id = $id;
		$post = $this->Post->read();
		$comment = $this->PostComment->find('all',array(
			'conditions'=>array('posts_id'=>$id),
			'order'=>array('PostComment.id DESC')
		));
		
		$set = array('post'=>$post,'comment'=>$comment);
